...
const AppSubmenu = (props) => {
    ...
    const renderLink = (item, i) => {
        ...
        if (item.to) {
            return (
                <NavLink aria-label={item.label} onKeyDown={onKeyDown} role="menuitem" className={({ isActive }) => (isActive ? `router-link-active  router-link-exact-active p-ripple` : 'p-ripple')} to={item.to} onClick={(e) => onMenuItemClick(e, item, i)} exact target={item.target}>
                    {content}
                </NavLink>
            )
        }
